...
namespace Fase06RepositoryCsv.Tests
{
    // Testes de integração para CSV Repository
    public static class TestesRepositorioCsv
    {
        public static void ExecutarTodos()
        {
            Console.WriteLine("=== TESTES DO REPOSITORY CSV ===\n");

            Teste1_ListAll_WhenFileDoesNotExist_ShouldReturnEmpty();
            Teste2_Add_Then_ListAll_ShouldPersistInFile();
            Teste3_Add_WithCommaAndQuotes_ShouldEscapeCorrectly();
        ...
        {
            return new CsvBookRepository(path);
        }
        ...
        private static void Teste3_Add_WithCommaAndQuotes_ShouldEscapeCorrectly()
        {
            Console.WriteLine("Teste 3: Add_WithCommaAndQuotes_ShouldEscapeCorrectly");
            ...
            try
            {
                ...
                var all = repo.ListAll();
                Verificar(all.Count == 1, "Deveria ter 1 livro");
                Verificar(all[0].Title.Contains(","), "Título deveria conter vírgula");
                Verificar(all[0].Author.Contains("\""), "Autor deveria conter aspas");
